...
class FinancialController extends ChangeNotifier {
  ...
  String fromDate = DateTime(DateTime.now().year - 1, DateTime.now().month,
  ...
  Future<void> getIOComes() async {
    isLoading = true;
    notifyListeners();

    try {
      IOComeList =
          await FinancialServices().getIOComes(from: fromDate, to: toDate);
      totalIncomeBalance = 0.0;
      totalOutcomeBalance = 0.0;
      totalBalance = 0.0;

      IOComeList.forEach((element) {
        if (element.type == 1) {
  ...
  Future<void> saveIOCome(BuildContext context) async {
    ...
    try {
      status =
          await FinancialServices().saveIOCome(selectedIOCome: selectedIOCome);
    } catch (e) {
  ...
  Future<void> selectDate(BuildContext context) async {
    DateTime selectedDate = await showDatePicker(
            context: context,
            initialDate:
                isHaveData ? selectedIOCome.happenDate : DateTime.now(),
